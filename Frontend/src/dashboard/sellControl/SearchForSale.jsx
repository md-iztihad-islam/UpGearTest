...
const SearchForSale = ({ 
  ...
  className = "",
  autoFocus = false,
  onSelectProduct = null, // ✅ NEW: if provided, calls this instead of navigating
}) => {
  ...
  const handleProductClick = (product) => {
    // ✅ If onSelectProduct prop is provided, use it (e.g. Sell page)
    if (onSelectProduct) {
        ...
        >
          ...
          {!isSearching && searchResults.length > 0 && (
            <div className="overflow-y-auto max-h-[calc(100vh-120px)] sm:max-h-[450px]">
              ...
              {searchResults.map((product, index) => (
                ...
                >
                  <Avatar className="h-12 w-12 sm:h-14 sm:w-14 rounded-md border border-gray-700 flex-shrink-0">
                    <AvatarImage src={product.bannerImage || product.images?.[0]} alt={product.title} className="object-cover" />
                  ...
                  <div className="flex-1 min-w-0 text-left">
                    ...
                    <div className="flex items-center gap-1.5 sm:gap-2 mt-1">
                      <span className="text-xs sm:text-sm font-semibold text-white">
                        {formatPrice(product.finalPrice || product.mainPrice || product.price)}
                      </span>
              ...
              {/* Only show "View All" if NOT in select mode */}
              {!onSelectProduct && (
