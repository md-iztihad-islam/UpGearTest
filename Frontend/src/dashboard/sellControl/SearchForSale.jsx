...
import { Search, X, TrendingUp, ChevronRight, Loader2, PackageX } from "lucide-react";
...
// Sums (remaining - reserved) across a product's "available"-status stocks.
// This is the sellable quantity right now, not just what's physically on the shelf.
...
const SearchForSale = ({
  ...
  const debouncedSearch = useCallback(
    debounce(async (query) => {
      ...
      try {
        ...
        // Fixed shape from the corrected backend: { success, data: { products, page, limit } }
  ...
  const handleInputChange = (e) => {
    ...
    setHighlightedIndex(-1);
    if (value.trim().length >= 2) debouncedSearch(value);
    else {
      setSearchResults([]);
      setIsSearching(false);
    }
  ...
  const handleProductClick = (product) => {
    if (onSelectProduct) {
      // In the selling flow, a product with no sellable stock shouldn't be selectable.
    ...
    switch (e.key) {
      ...
      case "Enter":
        ...
        if (highlightedIndex >= 0 && searchResults[highlightedIndex]) {
          handleProductClick(searchResults[highlightedIndex]);
  ...
  return (
    <div ref={searchRef} className={`relative ${className}`}>
      <div className="relative">
        ...
        />
        {searchQuery && (
          <button onClick={clearSearch} className="absolute right-2.5 sm:right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-white transition-colors z-10">
      ...
      {showDropdown && (
        <div className="fixed sm:absolute left-0 right-0 sm:left-auto sm:right-auto top-[52px] sm:top-full sm:mt-2 bg-gray-900 border-t sm:border border-gray-700 sm:rounded-lg shadow-2xl shadow-black/50 max-h-[calc(100vh-60px)] sm:max-h-[500px] overflow-hidden z-50 sm:min-w-[400px] lg:min-w-[500px]">
          {isSearching && (
          ...
          {!isSearching && searchResults.length > 0 && (
            <div className="overflow-y-auto max-h-[calc(100vh-120px)] sm:max-h-[450px]">
              ...
              {searchResults.map((product, index) => {
                ...
                const disabledForSelect = onSelectProduct && !inStock;
                return (
                  <button
                    key={product.productId}
                    onClick={() => handleProductClick(product)}
                    disabled={disabledForSelect}
                    className={`w-full flex items-center gap-2 sm:gap-3 p-2 sm:p-3 transition-colors border-b border-gray-800/50 last:border-0 ${
                      disabledForSelect ? "opacity-50 cursor-not-allowed" : "hover:bg-gray-800/70 active:bg-gray-800"
                  ...
                  >
                    <Avatar className="h-12 w-12 sm:h-14 sm:w-14 rounded-md border border-gray-700 flex-shrink-0">
                      <AvatarImage src={product.bannerImage || product.images?.[0]?.url} alt={product.title} className="object-cover" />
                      <AvatarFallback className="bg-gray-800 text-gray-400 rounded-md text-sm">
                        {product.title?.charAt(0).toUpperCase()}
                      </AvatarFallback>
                    </Avatar>
                    <div className="flex-1 min-w-0 text-left">
                      <p className="text-xs sm:text-sm font-medium text-white truncate leading-tight">{product.title}</p>
                      {product.subTitle && (
                        <p className="text-xs text-gray-400 truncate mt-0.5 leading-tight">{product.subTitle}</p>
                      )}
                      <div className="flex items-center gap-1.5 sm:gap-2 mt-1 flex-wrap">
                        <span className="text-xs sm:text-sm font-semibold text-white">{formatPrice(product.price)}</span>
                        {product.discount > 0 && (
                          <>
                            <span className="text-xs text-gray-500 line-through">{formatPrice(product.mainPrice)}</span>
                            <Badge className="text-xs px-1 py-0 h-4 sm:h-5 bg-red-600 text-white border-0">
                              -{Math.round((product.discount / product.mainPrice) * 100)}%
                            </Badge>
                          </>
                        )}
                        <Badge
                          className={`text-xs px-1.5 py-0 h-4 sm:h-5 border-0 ${
                            inStock ? "bg-green-600/80 text-white" : "bg-gray-600 text-gray-200"
                          }`}
                        >
                          {inStock ? `In Stock (${availableQty})` : "Out of Stock"}
                        </Badge>
                      </div>
                    </div>
                    {!inStock && onSelectProduct ? (
                      <PackageX className="h-4 w-4 text-gray-500 flex-shrink-0" />
                    ) : (
          ...
          {!isSearching && !searchQuery && recentSearches.length > 0 && (
            <div className="overflow-y-auto max-h-[calc(100vh-120px)] sm:max-h-[450px]">
              <div className="p-2 sm:p-3 border-b border-gray-800 flex items-center justify-between bg-gray-900/80 backdrop-blur-sm sticky top-0 z-10">
                <p className="text-xs font-medium text-gray-400 uppercase tracking-wide">Recent Searches</p>
                <button
              ...
              {recentSearches.map((search, index) => (
                ...
                >
                  ...
                  <button onClick={(e) => removeRecentSearch(search, e)} className="p-1 text-gray-500 hover:text-white opacity-0 group-hover:opacity-100 transition-all flex-shrink-0">
          ...
          {!isSearching && !searchQuery && recentSearches.length === 0 && (
            <div className="p-3 sm:p-4">
              <p className="text-xs font-medium text-gray-400 uppercase tracking-wide mb-2 sm:mb-3">Popular Searches</p>
