...
import shipMultipleAcceptedOrdersApi from "@/services/dashboard/order/shipMultipleAcceptedOrdersApi";
import cancelMultipleAcceptedOrdersApi from "@/services/dashboard/order/cancelMultipleAcceptedOrdersApi";
import { useMutation, useQuery, useQueryClient } from "@tanstack/react-query";
import { useNavigate } from "react-router-dom";
import { useState } from "react";
import { ArrowLeft, Eye, Download, Calendar, Loader2, FileText, Truck, X, CheckSquare, Square } from "lucide-react";
...
function AcceptedOrders() {
  ...
  const { data: acceptedOrdersData, isLoading } = useQuery({
    queryKey: ["acceptedOrdersData", startDate, endDate],
    // Real signature is (startDate, endDate) — no page/limit, backend doesn't paginate this.
    queryFn: () => getAcceptedOrdersApi(startDate, endDate),
  });

  // Controller returns { message, orders: [...] } — no data/pagination keys.
  const orders = acceptedOrdersData?.orders || [];

  const selectedOrders = orders.filter((o) => selectedIds.has(o.orderId));

  // orderProducts is ONE ROW PER UNIT (see addOrderService), each with a
  // populated `product` relation — not `products`/`productId`/`productQuantity`.
  ...
  const getProductWeight = (op) => {
    const specs = op.product?.specifications;
    if (!specs) return 0.5;
  ...
  // Exports the SELECTED orders only, not the whole list.
  const exportSelectedToCSV = () => {
    ...
    a.href = url;
    a.download = `accepted_orders_selected_${new Date().toISOString().split("T")[0]}.csv`;
  ...
  // Opens the invoice for every selected order that has one. Browsers may
  // block more than a handful of popups fired synchronously in a loop —
  // if that turns out to be an issue in practice, this should become a
  // "download all as zip" server endpoint instead.
  ...
  console.log("Invoices for selected orders:", selectedOrders.map((o) => o.invoiceURL));
  ...
  const { mutate: bulkAction, isPending: isUpdating } = useMutation({
    mutationFn: async (action) => {
      ...
      const apiFn = action === "ship" ? shipMultipleAcceptedOrdersApi : cancelMultipleAcceptedOrdersApi;
      return apiFn(ids);
    },
    onSuccess: () => {
      setSelectedIds(new Set());
      queryClient.invalidateQueries({ queryKey: ["acceptedOrdersData"] });
    },
    onError: (error) => {
      alert(error?.response?.data?.message || "Some orders failed to update, please retry");
      queryClient.invalidateQueries({ queryKey: ["acceptedOrdersData"] });
    },
  });
  ...
  const handleShipSelected = () => {
    ...
    if (!window.confirm(`Mark ${selectedIds.size} order(s) as shipped?`)) return;
    bulkAction("ship");
  };
    ...
    <div className="min-h-screen bg-black text-white p-4 sm:p-6 lg:p-10">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-12">
          ...
          <p className="text-gray-400 text-lg">View and manage accepted orders</p>
        </div>
          ...
          <div className="sticky top-4 z-10 mb-6 flex flex-col sm:flex-row items-center justify-between gap-3 bg-blue-950/60 border border-blue-500/40 rounded-xl px-6 py-4 backdrop-blur">
            ...
            <div className="flex flex-wrap justify-center gap-3">
              ...
              <button
                ...
                disabled={isUpdating}
                className="flex items-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-700 disabled:bg-gray-700 disabled:cursor-not-allowed text-white rounded-lg transition-colors duration-200 font-medium"
              >
        ...
        <div className="bg-gradient-to-br from-gray-900 to-gray-800 border border-gray-700 rounded-2xl overflow-hidden">
          ...
          <div className="px-6 py-4 border-b border-gray-700 flex items-center gap-3">
            ...
            <h2 className="text-sm font-semibold text-gray-400 uppercase tracking-wide">
              All Accepted Orders ({orders.length})
            </h2>
          </div>
                  ...
                  >
                    <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                      ...
                      <div className="flex items-start gap-4 flex-1">
                        ...
                        <div className="flex-shrink-0 w-10 h-10 rounded-full bg-green-600 flex items-center justify-center text-white font-semibold">
                          {idx + 1}
                        </div>
